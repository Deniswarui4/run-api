...
func (h *AttendeeHandler) InitiateTicketPurchase(c *gin.Context) {
	...
	if err := h.db.First(&ticketType, "id = ? AND event_id = ?", req.TicketTypeID, req.EventID).Error; err != nil {
	...
	if !ticketType.IsAvailable() {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Tickets not available for sale"})
		return
	}

	// Check quantity
	if req.Quantity > ticketType.RemainingTickets() {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Not enough tickets available"})
		return
	}

	if req.Quantity > ticketType.MaxPerOrder {
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("Maximum %d tickets per order", ticketType.MaxPerOrder)})
		return
	}

	// Calculate total amount
	totalAmount := ticketType.Price * float64(req.Quantity)
	...
	transaction := &models.Transaction{
		...
		Description:      fmt.Sprintf("Purchase of %d ticket(s) for %s", req.Quantity, event.Title),
	}
...
func (h *AttendeeHandler) VerifyPayment(c *gin.Context) {
	...
	ticketTypeID, _ := metadata["ticket_type_id"].(string)
	
	// Handle quantity - could be float64 or string
	var quantity int
	switch v := metadata["quantity"].(type) {
	case float64:
		quantity = int(v)
	case string:
	...
	default:
		quantity = 1 // Default to 1 if type is unknown
	}
